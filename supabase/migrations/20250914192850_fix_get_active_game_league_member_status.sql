-- Fix get_active_game_for_league function to check for 'approved' status instead of 'active'
-- League members have status 'approved', not 'active'
...
BEGIN
  ...
  if not exists (
    ...
    where lm.league_id = p_league_id
      ...
      and lm.status = 'approved' -- Fixed: was 'active', should be 'approved'
  ) then
...
SELECT 'get_active_game_for_league function status check fixed' as status;
-- Fix 409 conflict from invite_code UNIQUE constraint
-- Issue: invite_code generation can rarely collide, causing 409 errors
-- Solution: Use a loop with retry logic to generate unique invite codes
...
BEGIN
  ...
  -- Create the league with the unique invite code
...
'Creates a new league with the caller as owner. Uses retry logic to generate unique invite codes and proper auth.uid() → profiles.id mapping.';

SELECT 'Fixed create_league_with_owner to handle invite_code conflicts' as status;
